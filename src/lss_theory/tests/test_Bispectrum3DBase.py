...
info['cosmo_par_file'] =  os.path.join(input_dir, 'cosmo_pars/planck2018_fiducial.yaml')
info['cosmo_par_fid_file'] = os.path.join(input_dir, 'cosmo_pars/planck2018_fiducial.yaml')
info['survey_par_file'] = os.path.join(input_dir, 'survey_pars/survey_pars_v27_base_cbe_Mar21.yaml')
...
info['plot_dir'] = os.path.join(test_dir, 'plots/theory/bispectrum/')

@pytest.mark.debug
@pytest.mark.parametrize("info, cosmo_par_file, fn_expected", \
    [ \
        ( \
            info, \
            os.path.join(input_dir, 'cosmo_pars/planck2018_fiducial.yaml'), \
            os.path.join(data_dir, 'bis_base_fnl_0.npy') \
        ), \
        ( \
            info, \
            os.path.join(input_dir, 'cosmo_pars/planck2018_fnl_1p0.yaml'), \
            os.path.join(data_dir, 'bis_base_fnl_1.npy') \
        ), \
    ] \
) 
