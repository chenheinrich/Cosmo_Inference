...
class B3D(DataVector):
    ...
    def _get_triangle_spec(self):
        return TriangleSpec(self._data_spec.k)
...
class B3D_RSD(B3D): #No AP
    ...
    def __init__(self, cosmo_par, cosmo_par_fid, survey_par, b3d_spec):
        ...
        self._triangle_spec = TriangleSpecTheta1Phi12(\
            self._data_spec.k, self._data_spec.theta1, self._data_spec.phi12)
        print('self._triangle_spec.nori', self._triangle_spec.nori)
        print('self._triangle_spec.theta1', self._triangle_spec.theta1)

    def _get_triangle_spec(self):
        return TriangleSpecTheta1Phi12(\
            self._data_spec.k, self._data_spec.theta1, self._data_spec.phi12)
    ...
    def _get_Bggg_b10_at_iz(self, iz, isample1, isample2, isample3):
        ...
        sigp = 4.0 # TODO need to specify centrally
        k1, k2, k3 = self._get_k1_k2_k3_array(iz)

        for iori in range(self._triangle_spec.nori):
            
            bias = self._grs_ingredients.get('galaxy_bias') 
            f_at_z = self._grs_ingredients._get_f()[iz] #TODO need to think about doing this in ingredients or not?
            Bmmm = self._get_Bmmm()
            ...
            Z1_k1 = bias[isample1, iz, self._ik1, imu] + f_at_z * mu1 ** 2
            Z1_k2 = bias[isample2, iz, self._ik2, imu] + f_at_z * mu2 ** 2
            Z1_k3 = bias[isample3, iz, self._ik3, imu] + f_at_z * mu3 ** 2

            kmu_squared = k1 * k1 * mu1 * mu1 + k2 * k2 * mu2 * mu2 + k3 * k3 * mu3 * mu3
            arg = -0.5 * kmu_squared * sigp*sigp
            fog = np.exp(arg)

            Bggg_b10[:, iori] = Bmmm[iz, :] *  Z1_k1 * Z1_k2 * Z1_k3 * fog
